...
@section('title', $config['title'] ?? 'Instagram Downloader')
    ...
    <div class="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <h2 class="text-xl sm:text-2xl lg:text-3xl font-bold mb-6 slide-up" style="animation-delay: 0.2s">
            <span class="bg-gradient-to-r from-violet-600 via-pink-500 to-orange-400 bg-clip-text text-transparent">
                {{ $config['hero_highlight'] ?? 'Download in HD Quality' }}
            </span>
        </h2>
        ...
        <p class="text-lg text-gray-600 dark:text-gray-400 mb-10 max-w-2xl mx-auto slide-up"
            ...
            {{ $config['subtitle'] ?? 'Download Instagram content easily. Free, fast, and no login required.' }}
        </p>
            ...
            <form id="downloadForm" class="relative flex flex-col sm:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" id="urlInput" name="url"
                        placeholder="{{ $config['placeholder'] ?? 'Paste Instagram URL here...' }}"
        ...
        <div class="flex flex-wrap justify-center gap-3 mt-8 slide-up" style="animation-delay: 0.5s">
            @foreach ($config['formats'] as $format)
                <span class="px-3 py-1.5 rounded-lg bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 text-sm font-medium shadow-sm border border-gray-100 dark:border-gray-700">
                    {{ $format }}
                </span>
            @endforeach
        </div>
...
<section class="py-16 bg-white dark:bg-gray-900">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            @if ($pageType !== 'reels' && \App\Models\Page::where('slug', 'reels')->where('is_active', true)->exists())
            ...
            @if ($pageType !== 'video' && \App\Models\Page::where('slug', 'video')->where('is_active', true)->exists())
            ...
            @if ($pageType !== 'photo' && \App\Models\Page::where('slug', 'photo')->where('is_active', true)->exists())
            ...
            @if ($pageType !== 'story' && \App\Models\Page::where('slug', 'story')->where('is_active', true)->exists())
            ...
            @if ($pageType !== 'carousel' && \App\Models\Page::where('slug', 'carousel')->where('is_active', true)->exists())
